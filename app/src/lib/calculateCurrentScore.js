...
// Dictionary cache to avoid repeated fetches
...
const isValidWord = async (word) => {
  try {
    ...
    if (!dictionaryCache) {
      ...
      dictionaryCache = text.toLowerCase().split('\n').map(word => word.trim());
    }
    
    // Check if the word is in the dictionary
    return dictionaryCache.includes(word.toLowerCase());
  } catch (error) {
...
// Main function to analyze the board and calculate current score
const calculateCurrentScore = async (gameState) => {
  const { placedTiles, bonusTilePositions, letterPoints } = gameState;
  const boardSize = 5; // Assuming a 5x5 board
  
  // If no tiles are placed, return empty results
  ...
  // If no words are formed (only single tiles), return empty results
  ...
  // Process all words in parallel for better performance
  ...
  // Calculate total score and build results array
